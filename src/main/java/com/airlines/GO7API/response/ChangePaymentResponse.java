...
public class ChangePaymentResponse {
    ...
    public static ChangePaymentRspDto generateResponse(ChangePaymentRspGo7Dto go7Response,
            ...
            for (ChangePaymentRspGo7Dto.Passenger go7Pax : go7PaxList) {
                ...
                if (go7Pax.getETickets() != null && go7Pax.getETickets().getFlight() != null) {
                    ...
                    for (ChangePaymentRspGo7Dto.Passenger.ETicketFlight etf : go7Pax.getETickets().getFlight()) {
                        ...
                        // tdi.setPaxId(List.of(paxId)); // Using Collections.singletonList for older
                        // java compat if needed, or Arrays.asList
                        ...
                        doc.setDateOfIssue(formatDate(LocalDate.now().toString())); // Today
        ...
        if (flightList != null) {
            ...
            for (ChangePaymentRspGo7Dto.Flight f : flightList) {
                ...
                if (f.getTaxes() != null) {
                    addTaxToList(taxList, "Ground_handling", f.getTaxes().getGroundHandling(), itemCurrency);
                    addTaxToList(taxList, "Security", f.getTaxes().getSecurity(), itemCurrency);
                    addTaxToList(taxList, "Fuel", f.getTaxes().getFuel(), itemCurrency);
                    addTaxToList(taxList, "tax_4", f.getTaxes().getTax_4(), itemCurrency);
                }
    ...
    private static void addTaxToList(List<ChangePaymentRspDto.OrderItemsDTO.Tax> list, String description,
            double amount,
            String currency) {
        if (amount <= 0.001)
            return;
